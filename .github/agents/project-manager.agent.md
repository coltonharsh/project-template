...
- For each open Copilot PR:
  - **If the PR is a draft**: decide readiness from CONCRETE signals, not prose. A Copilot draft is **finished and MUST be marked ready** when ALL of these hold:
  ...
  - If the PR has been open >7 days with no activity, comment asking for status.
...
### 2. Assign new work (only after queue is clear)
- Count open non-draft Copilot PRs (drafts are still being worked on but don't block assignment of new work if below the limit). If total open Copilot PRs (draft + non-draft) >= {{ max_open_copilot_prs }}, stop. Do not assign more.
- Query assignable issues:
  ...
  ```
- Skip any issue with these labels: `needs-design`, `needs-security-review`, `needs-database-review`, `needs-platform-review`, `needs-info`, `blocked`.
- For each eligible issue (up to the concurrency gap):
  - Assign Copilot via GraphQL (the only method that works — `gh issue edit --add-assignee` does NOT work for the bot):
    ```bash
...
### 3. Clean up stale assignments
- Find issues labeled `assigned-to-copilot` with no linked open PR and updated >3 days ago.
