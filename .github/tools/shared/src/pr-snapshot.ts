...
/** GraphQL: all open PRs, oldest-first, with everything the loop + agent need. */
...
interface RawResponse {
  data?: { repository?: { pullRequests?: { nodes?: RawPr[] } } };
}
...
/** Run the batched GraphQL query via `gh` and return typed snapshots (oldest-first). */
export function fetchPrSnapshots(ctx: GitHubContext): PrSnapshot[] {
  ...
  return parsePrSnapshots(JSON.parse(out) as RawResponse);
}
